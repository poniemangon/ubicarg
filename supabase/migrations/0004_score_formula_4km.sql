...
--
-- Scoring formula, take 2: 100 pts within 1km unchanged, but the falloff
-- beyond that eases from -1 pt/500m to -1 pt/4km — 500m/pt made even good
-- country-scale guesses (tens of km off) score low (e.g. 33.6km only
-- scored 35/100). Client-side match: scoreForDistance() in src/App.jsx.
-- Self-contained — safe to run whether or not 0002_score_formula_1km.sql
-- ran first, since create or replace just redefines these from scratch.
...
begin
  ...
  for idx in 1 .. num_rounds loop
    ...
    distance := case when points >= 100 then round(random() * 1000) else 1000 + (100 - points) * 4000 + floor(random() * 4000) end;
...
begin
  ...
  while i + 1 <= n loop
    ...
    for j in 1 .. 5 loop
      ...
      distance := case when pts >= 100 then round(random() * 1000) else 1000 + (100 - pts) * 4000 + floor(random() * 4000) end;
